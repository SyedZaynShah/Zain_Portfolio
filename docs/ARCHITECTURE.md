...
│   ├── painters/              #   particle_painter, glow_painter (CustomPainters)
│   └── widgets/               #   custom_cursor, hover_card, cta_button,
...
- **`shared`** holds the design system's widget vocabulary (glass, glow, badges, reveals).
  ...
  the Contact form can share the same `HoverCard` or `CtaButton` without coupling.
...
──────────────────────────── runtime, pointer-driven ───────────────────────────
 ...
 CursorTarget(MouseRegion onEnter/onExit) — sprinkled on every interactive widget
      └─► CursorController.intent   : ValueNotifier<CursorIntent> idle|link|view
