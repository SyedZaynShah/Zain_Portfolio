...
can't blow the 100 MB image cache. The `GlowOrb` painter remains available as a
design-system primitive (`lib/shared/painters/`), tree-shaken when unused.
...
repaints would mark the shared layer dirty and force the glow and noise to
re-rasterize 60 times a second for no visual change.
...
3. `_NoisePainter` tiles it with `paintImage(repeat: ImageRepeat.repeat,
   ...
   after first decode).
4. Opacity (~3 %) is applied per-draw via `paintImage`'s `opacity` argument —
   the tiles never overlap, so this is identical to a layer opacity but avoids
   a full-screen `saveLayer`. The whole thing sits in `IgnorePointer` +
   `RepaintBoundary` at the top of the shell stack.
...
- **`FloatingNav`** (`lib/features/shell/floating_nav.dart`) — one pill, only
  visible after scrolling past the hero. This is the app's **only**
  `BackdropFilter`.
...
When adding UI: do not introduce new `BackdropFilter`s. If you genuinely need a
glass surface, factor a single shared widget and reuse it — never stack many
blur layers in one viewport (each is a separate saveLayer). Verify with:
...
- Backdrop blur is disproportionately expensive on integrated/mobile GPUs.
  If targeting very low-end devices becomes a goal, gate the floating nav's
