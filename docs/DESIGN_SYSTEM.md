...
| Token | Hex (AARRGGBB) | Effective alpha | Usage |
|---|---|---|---|
| `accentSecondary` | `#FF7B5CF0` | 100 % | `ColorScheme.secondary`; the secondary accent (deep violet), used sparingly. |
...
| `borderActive` | `#4000F5C4` | 25.1 % teal | Active/hover/focus borders — hovered `TechBadge`, cards, and interactive controls. |
| `glassWhite` | `#0FFFFFFF` | ~5.9 % white | Base fill of glass surfaces (e.g. the floating nav pill). |
...
|---|---|---|
| `none` | 0 | — |
| `xs` | 4 | Scrollbar radius, cursor dot corners. |
| `sm` | 8 | Tooltips. |
...
- **`CursorIntent`** enum — `idle` (6 px dot + 32 px ring), `link` (ring grows to 48 px, fills
  `accentSubtle`), `view` (64 px ring, `accentGlow` border, `VIEW` label inside — used on project
  cards).
...
`borderSubtle` border; label is `AppTypography.badge` upper-cased. Informational (not tappable):
on hover the fill → `accentSubtle`, border → `borderActive`, text → accent, with no scale so it
never reads as a button. Prop: `label`.
...
| Cursor dot morph | `instant` (100 ms) | — |
