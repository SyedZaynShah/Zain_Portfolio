...
class _AbstractArtPanelState extends State<AbstractArtPanel>
  ...
  void initState() {
    super.initState();
    _controller = AnimationController(vsync: this, duration: AppDurations.crawl)
      ..repeat(reverse: true);
  }
  ...
  Widget build(BuildContext context) {
    final pulse =
        context.reducedMotion ? const AlwaysStoppedAnimation(0.5) : _controller;
    return Stack(
      fit: StackFit.expand,
      children: [
        AnimatedBuilder(
          ...
          builder:
              (context, _) => CustomPaint(
                painter: _GallerySpotlightPainter(
            ...
            children: [
              ...
              Text(
                ...
                style: AppTypography.caption.copyWith(
                  color: widget.project.accentColor.withValues(alpha: 0.8),
                ),
              ),
            ],
          ),
...
class _GallerySpotlightPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      final sweep =
          i == 0
              ? math.pi * 2 * (0.55 + 0.18 * pulse)
              : math.pi * 2 * (0.9 - i * 0.1);
      canvas.drawArc(
        Rect.fromCircle(center: spot, radius: r),
        -math.pi / 2,
        sweep,
        false,
        ringPaint,
  ...
  @override
  bool shouldRepaint(_GallerySpotlightPainter old) =>
