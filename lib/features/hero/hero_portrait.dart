...
class _HeroPortraitState extends State<HeroPortrait>
  ...
  /// True once a real mouse hover has occurred anywhere in the shell. Until
  /// then the reveal stays on its autonomous orbit — which is what keeps the
  /// colour discoverable on touch tablets wider than 768px, where the width
  /// heuristic alone wrongly reports hover support.
  bool _sawMouse = false;
  ...
  void initState() {
    ...
    _sawMouse = CursorController.visible.value;
    CursorController.visible.addListener(_onMouseVisibility);
  }
  ...
  void dispose() {
    ...
    CursorController.visible.removeListener(_onMouseVisibility);
    _breath.dispose();
    _orbit.dispose();
    _enter.dispose();
    super.dispose();
  ...
  // Autonomous (orbiting) on touch devices and until a real mouse is seen.
  bool get _autonomous => !context.supportsHover || !_sawMouse;

  /// First real mouse hover: hand off from the autonomous orbit to the
  /// pointer-driven reveal. Seed the ramp from the current pointer so the
  /// radius doesn't pop — full if the cursor is already over the hero,
  /// otherwise hidden until the first hover, as on desktop.
  void _onMouseVisibility() {
    if (!mounted || _sawMouse || !CursorController.visible.value) {
      return;
    }
    setState(() => _sawMouse = true);
    _enter.value = widget.pointer.value != null ? 1.0 : 0.0;
  }
...
/// Tiny inline hint shown once near the portrait. Its copy tracks the actual
/// interaction: `hover the portrait` once a mouse is present, otherwise
/// `watch the colour drift` — matching the autonomous orbit shown to
/// touch/no-mouse visitors (where tapping does nothing).
  ...
  Widget build(BuildContext context) => Row(
    ...
    children: [
      ...
      ValueListenableBuilder<bool>(
        ...
        builder:
            (context, mouseSeen, _) => Text(
              mouseSeen ? 'hover the portrait' : 'watch the colour drift',
