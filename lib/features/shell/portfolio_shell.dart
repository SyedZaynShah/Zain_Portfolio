...
class _PortfolioShellState extends State<PortfolioShell> {
  ...
  void _scrollToSection(int index) {
    ...
    final target = (_scroll.offset +
            ...
            (index == 0 ? 0 : Spacing.xxl))
        .clamp(0.0, _scroll.position.maxScrollExtent);
    final distance = (target - _scroll.offset).abs();
    _scroll.animateTo(
      target,
      duration: Duration(
        milliseconds: (600 + distance * 0.25).clamp(600, 1800).round(),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    // The hero-snap "chapter turn" is a wheel affordance; on the mobile layout
    // it fights native touch momentum, so fall back to plain physics there.
    final content = SingleChildScrollView(
      controller: _scroll,
      physics:
          context.isMobile
              ? const BouncingScrollPhysics()
              : const HeroSnapScrollPhysics(parent: BouncingScrollPhysics()),
      child: Column(
        children: [
          ...
          RepaintBoundary(
            child: SizedBox(
              ...
              child: ValueListenableBuilder<bool>(
                ...
                builder:
                    (context, heroVisible, child) =>
                        TickerMode(enabled: heroVisible, child: child!),
                child: HeroSection(
                  play: _introDone,
          ...
          RepaintBoundary(
            child: KeyedSubtree(
              ...
              child: WorksSection(
                ...
                onStartProject: () => _scrollToSection(_contactIndex),
              ),
            ),
          ),
          RepaintBoundary(
          ...
          RepaintBoundary(
            child: KeyedSubtree(
              key: _sectionKeys[4],
              child: StatementSection(scrollController: _scroll),
            ),
          ),
          RepaintBoundary(
            child: KeyedSubtree(
    ...
    Widget page = Stack(
      children: [
        ...
        if (!_introDone)
          Positioned.fill(child: IntroScreen(onFinished: _onIntroFinished)),
