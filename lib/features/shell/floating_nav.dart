...
class _FloatingNavState extends State<FloatingNav> {
  bool _menuOpen = false; // requested-open
  bool _menuMounted = false; // in the tree during open + exit animation
  ...
  Widget build(BuildContext context) => ValueListenableBuilder<bool>(
    ...
    builder: (context, visible, _) {
      ...
      // A resize back to the desktop layout removes the hamburger; close the
      // fullscreen menu so it can't be stranded with no way out.
        ...
        children: [
          if (_menuMounted)
            _MobileMenu(
              ...
              onTap: _go,
              onClose: _closeMenu,
              onClosed: () => setState(() => _menuMounted = false),
            ),
          Align(
  ...
  Widget _bar(BuildContext context, bool compact) => ClipRRect(
        ...
        child: Row(
          ...
          children: [
            ...
            if (compact) ...[
              ...
              CursorTarget(
                child: IconButton(
                  onPressed: () => _menuOpen ? _closeMenu() : _openMenu(),
                  icon: AnimatedSwitcher(
                    ...
                    child: Icon(
                      _menuOpen ? LucideIcons.x : LucideIcons.menu,
...
class _NavLinkState extends State<_NavLink> {
  ...
  Widget build(BuildContext context) {
    final highlight = widget.active || _hovered || _focused;
    return CursorTarget(
      child: Semantics(
        button: true,
        label: widget.label,
          ...
          child: Padding(
            ...
            child: AnimatedScale(
              ...
              child: Icon(
                ...
                size: 17,
                color:
                    _hovered ? AppColors.textAccent : AppColors.textSecondary,
              ),
            ),
          ),
        ),
...
class _MobileMenu extends StatefulWidget {
  ...
  /// Requested state. Flipping false reverses the reveal instead of cutting.
  final bool open;
  final List<NavItem> items;
  final ValueChanged<int> onTap;

  /// Request a close (e.g. tapping the backdrop).
  final VoidCallback onClose;

  /// The exit animation finished — the parent may now unmount this widget.
...
class _MobileMenuState extends State<_MobileMenu>
  ...
  @override
  void didUpdateWidget(_MobileMenu oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.open != oldWidget.open) {
      if (widget.open) {
        _reveal.forward();
      } else if (context.reducedMotion) {
        _reveal.value = 0; // dismissed status → onClosed
      } else {
        _reveal.reverse();
      }
    }
  ...
  Widget build(BuildContext context) => Positioned.fill(
    child: AnimatedBuilder(
      animation: _reveal,
      // Tapping empty space closes the menu; item/social taps win the arena
      // in their own areas.
      builder:
                              ...
                              GestureDetector(
                                ...
                                child: Padding(
                                  ...
                                  child: Text(
                                    ...
                                    style: AppTypography.monoAccent,
                                  ),
                                ),
                              ),
                          ],
