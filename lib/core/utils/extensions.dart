...
extension PortfolioContext on BuildContext {
  ...
  double get pageGutter =>
      responsive(mobile: Spacing.md, tablet: Spacing.xl, desktop: Spacing.xxl);

  /// Vertical padding between major sections at the current breakpoint.
  double get sectionSpacing => responsive(
    mobile: Spacing.sectionMobile,
    tablet: Spacing.sectionTablet,
...
extension UnitInterval on double {
  ...
  double subProgress(double start, double end) =>
      ((this - start) / (end - start)).clamp(0.0, 1.0);
}
