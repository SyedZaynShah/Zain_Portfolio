...
class ParticleField extends StatefulWidget {
  const ParticleField({super.key, this.count, this.opacityScale = 1.0});

  /// Particle count override; defaults to 70 desktop / 30 mobile.
  final int? count;

  /// Uniform multiplier on every particle/star alpha. Lets a caller dim the
  /// whole field without an Opacity layer, which would force a full-viewport
  /// saveLayer every animation frame.
...
class _ParticleFieldState extends State<ParticleField>
  ...
  void _ensureParticles(Size size) {
    ...
    _field
      ..bounds = size
      ..particles.addAll(
        List.generate(n, (_) => _Particle.spawn(_rng, size, anywhere: true)),
      );
  }

  /// Advances the model. No setState — the painter repaints off the ticker.
  ///
  /// Integration is scaled by real elapsed time so the field drifts at the
  /// same wall-clock speed on 60/120/144 Hz displays. Velocities are expressed
  /// in px per 60 Hz frame, so `f` normalizes them to the actual frame delta.
  void _advance() {
    ...
    // First tick, resume-from-background, and jank can produce huge or negative
    // deltas — fall back to a single nominal frame in those cases.
    if (dt <= 0 || dt > 1 / 30) {
      dt = 1 / 60;
    }
    final f = dt * 60; // 1.0 at 60 fps
  ...
  factory _ShootingStar.spawn(math.Random rng, Size size) {
    ...
    return _ShootingStar(
      start: Offset(fromLeft ? -40 : size.width + 40, y),
        ...
        fromLeft ? size.width + 40 : -40,
        y + size.height * (rng.nextDouble() * 0.3 - 0.15),
      ),
    );
  }
...
class _ParticleFieldPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final p in field.particles) {
      paint.color = (p.teal ? AppColors.accentPrimary : AppColors.textPrimary)
          .withValues(alpha: p.opacity * opacityScale);
    ...
    if (s != null) {
      ...
      final tail =
          Offset.lerp(
            s.start,
            s.end,
            Curves.easeIn.transform((s.t - 0.06).clamp(0.0, 1.0)),
          )!;
  ...
  bool shouldRepaint(covariant _ParticleFieldPainter oldDelegate) =>
      oldDelegate.field != field || oldDelegate.opacityScale != opacityScale;
}
