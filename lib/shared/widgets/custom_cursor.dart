...
class CursorController {
  ...
  static final ValueNotifier<CursorIntent> intent = ValueNotifier(
    CursorIntent.idle,
  );
  // Start offscreen so there is no (0,0) flash before the first mouse event.
  static final ValueNotifier<Offset> position = ValueNotifier(
...
class _CustomCursorOverlayState extends State<CustomCursorOverlay>
  ...
  bool _snapped = false; // false until the ring has been placed at the pointer
  ...
  void _wake() {
    // First real pointer position: place the ring there so it materialises at
    // the cursor instead of streaking in from the top-left corner.
    if (!_snapped) {
      _snapped = true;
      _ringPos = CursorController.position.value;
    }
    if (!_ticker.isActive) {
      _ticker.start();
    }
