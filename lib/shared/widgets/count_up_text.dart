...
class CountUpText extends StatefulWidget {
  const CountUpText(
    ...
    required this.style,
    super.key,
    this.duration = AppDurations.sluggish,
  });
...
class _CountUpTextState extends State<CountUpText>
  ...
  void didUpdateWidget(CountUpText oldWidget) {
    ...
    if (oldWidget.value != widget.value) {
      _parse();
      // A new value shows immediately at its final figure; it does not replay
      // the count-up on an in-place update (the reveal is a scroll-in effect).
      if (_triggered || context.reducedMotion) {
        _controller.value = 1;
      }
    }
  }
