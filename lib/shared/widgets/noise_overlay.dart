  ...
  Widget build(BuildContext context) {
    ...
    return IgnorePointer(
      child: RepaintBoundary(
        child: CustomPaint(
          painter: _NoisePainter(tile, widget.opacity),
          size: Size.infinite,
        ),
      ),
    );
...
class _NoisePainter extends CustomPainter {
  const _NoisePainter(this.tile, this.opacity);
  ...
  void paint(Canvas canvas, Size size) {
    // Alpha is applied per-draw here rather than via an Opacity widget — the
    // tiles never overlap, so it is visually identical and avoids a
    // full-screen saveLayer every time the overlay repaints.
    paintImage(
      canvas: canvas,
