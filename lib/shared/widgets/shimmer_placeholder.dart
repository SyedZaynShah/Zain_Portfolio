...
class _ShimmerPlaceholderState extends State<ShimmerPlaceholder>
  ...
  void didChangeDependencies() {
    ...
    // Sweep only when motion is allowed; reduced motion shows a static tint.
  ...
  DecoratedBox _sweep(double t) => DecoratedBox(
  ...
  Widget build(BuildContext context) {
    ...
    // Isolated in its own layer so the ticker-rate repaint doesn't re-record
    // the enclosing section while assets decode.
    return RepaintBoundary(
      child: AnimatedBuilder(
        animation: _controller,
        builder: (context, _) => _sweep(_controller.value),
      ),
    );
  }
}
