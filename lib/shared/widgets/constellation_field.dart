...
class _ConstellationFieldState extends State<ConstellationField>
  ...
  void _seed(Size size) {
    ...
    for (var c = 0; c < widget.clusters; c++) {
      ...
      for (var i = 0; i < widget.perCluster; i++) {
        _nodes.add(
          _Node(
            (cx + (_rng.nextDouble() - 0.5) * size.width * 0.4).clamp(
              0,
              size.width,
            ),
            (cy + (_rng.nextDouble() - 0.5) * size.height * 0.4).clamp(
              0,
              size.height,
            ),
  ...
  void _advance() {
    ...
    // Real elapsed time keeps the chase speed, damping equilibrium, and drift
    // identical on 60/120/144 Hz displays. [k] is 1.0 at 60 fps; the impulse
    // and position step scale by it, and the per-frame damping is corrected to
    // a per-second rate so terminal velocity doesn't shift with refresh rate.
    ...
    final k = dt * 60;
    final damp = math.pow(0.95, k).toDouble();
    _t += dt;
    ...
    for (final n in _nodes) {
      ...
      if (dist < 220) {
        final f = _pointerPull * (1 - dist / 220) / dist * k;
        n
          ..vx += toAx * f
          ..vy += toAy * f;
      }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      for (var j = i + 1; j < nodes.length; j++) {
        ...
        if (d < linkDist) {
          linkPaint.color = linkColor.withValues(
            alpha: 0.16 * (1 - d / linkDist),
          );
    ...
    if (p != null) {
      ...
      canvas.drawCircle(
        p,
        3.5,
        Paint()..color = linkColor.withValues(alpha: 0.9),
      );
    }
    // Nodes.
